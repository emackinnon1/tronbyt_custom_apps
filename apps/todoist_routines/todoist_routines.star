...
def get_tasks(token, params):
    ...
    # )
    rep = http.get("https://api.todoist.com/rest/v2/tasks?filter=yesterday", headers = {"Authorization": "Bearer %s" % token}, params = {"filter": "today | overdue"})
    ...
    if rep.status_code != 200:
        fail("Todoist request failed:", rep.status_code, rep.body())
